...
function SteeringTable:createVehicle(x,y,i)
  ...
  Vehicle.mass = 1.0
  Vehicle.maxSpeed = 500.0
  Vehicle.maxForce = 10.0
  Vehicle.steeringForce = vector(1.0,1.0)
  Vehicle.accel = vector(0.0,0.0)
  Vehicle.panicRadius = 100.0
  Vehicle.seekRadius = 1.0
  ...
  -- all steering behaviors should be accessible for any vehicle through this table
    ...
    function Vehicle.steeringBehaviors:pursuit(evader)
      ...
      --print("Vehicle.heading.x")
      --print(tostring(Vehicle.heading.x))
      local evaderheading = vector(0.0,0.0)
      evaderheading = evader.vel:normalized()
      local relativeHeading = Vehicle.heading.x*evaderheading.x + Vehicle.heading.y*evaderheading.y
      --print("relativeHeading")
      --print(tostring(relativeHeading))
      --print("toEvader x,y")
      --print(tostring(toEvader.x))
      --print(tostring(toEvader.y))
      
      local dotToEvaderSelf = Vehicle.heading.x*toEvader.x+Vehicle.heading.y*toEvader.y
      --print("dotToEvaderSelf")
      --print(tostring(dotToEvaderSelf))
    ...
    function Vehicle.steeringBehaviors:seek(target)
      ...
      --print("velCheck x,y, and seekRadius")
      --print(tostring(velCheck.x))
      --print(tostring(velCheck.y))
      --print(tostring(Vehicle.seekRadius^2))
